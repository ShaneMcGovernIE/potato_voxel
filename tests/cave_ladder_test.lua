...
local MOD_PATH = os.getenv("DS_MOD_PATH") or "mods/potato_voxel"
...
for setId, entry in pairs(spec.tilesets) do
  for class, tiles in pairs(entry) do
    if type(tiles) == "table" and type(tiles[1]) == "number"
      ...
      local into = (setId == "CAVERN") and dead or deadElsewhere
      into[#into + 1] = setId .. "." .. class
    end
  end
end
table.sort(dead)
table.sort(deadElsewhere)
T.eq(#dead, 0, "every class CAVERN pins is one TileShape resolves: "
  .. table.concat(dead, ", "))

-- The same sweep over the REST of the profile is reported rather than
-- asserted, because it already finds pins this change did not put there
-- and a ladder fix is the wrong place to decide what they should be.
-- `rail_face` is the live example: pinned on the gen2 tilesets, absent
-- from TileShape, so those tiles quietly fall through to detection.
if #deadElsewhere > 0 then
  print(("NOTE %d pin(s) elsewhere name a class TileShape does not "
         .. "resolve, so they do nothing: %s")
        :format(#deadElsewhere, table.concat(deadElsewhere, ", ")))
end
