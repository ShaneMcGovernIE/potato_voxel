-- Gen 2 palettes are selected per 8x8 tileset tile at draw time.  A voxel
-- mesh samples an atlas directly, so make a small private atlas with those
-- palette choices baked in before terrain is meshed.
local V = ...
...
local function sourcePixels(atlas, tileset)
  ...
  -- A roof-bearing atlas is composed by Gold at runtime and has no source
  -- path. Read it once rather than dropping the roof when voxel mode starts.
