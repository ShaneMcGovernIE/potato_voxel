...
function DayTint.install()
  ...
  function Renderer:endFrame(zones, worldZones)
    ...
    local ok, viewport = pcall(inner, self, zones, worldZones)
    gfx.draw = draw
    if not ok then error(viewport, 0) end
    -- the engine hands the frame's viewport back through this seam
    -- (Game:draw -> render.hud); pass it through with the tint applied
    return viewport
  end
