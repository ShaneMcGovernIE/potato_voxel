...
local RAY_SHADER = [[
  ...
  uniform Image sunMap2;     // the CAST layer: sprites only
  ...
  // the CAST layer, ANDed with the world's answer like the scene shader's
  // sunlight(): a thread of light dies where either layer has something
  // closer to the sun, so a character passing through a beam still breaks
  // it the way the single-map pass always did
  float castDepth(vec2 uv) {
    vec4 c = Texel(sunMap2, uv);
    return c.r + c.g * (1.0 / 255.0);
  }
    ...
    for (int i = 0; i < STEPS; i++) {
      ...
      if (p.y < fogW.z) {
        ...
        if (su.x > 0.0 && su.x < 1.0 && su.y > 0.0 && su.y < 1.0
          ...
          lit = mix(1.0,
                    step(su.z - sunBias, sunDepth(su.xy))
                    * step(su.z - sunBias, castDepth(su.xy)),
                    edge);
        }
