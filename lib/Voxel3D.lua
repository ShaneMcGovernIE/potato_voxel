...
local SHADER = [[
  ...
  vec4 position(mat4 transform_projection, vec4 vertex_position) {
    ...
    if (pull > 0.0) {
      // A contact blob can be directly under the camera on small screens.
      // Normalizing a zero-length ray produces NaNs on some mobile GPUs;
      // those NaNs turn the tiny decal into a triangle spanning the screen.
  ...
  float sunDepth(Image m, vec2 uv) {
    vec4 c = Texel(m, uv);
  ...
  //
  // TWO layers, ANDed per tap: the world layer (terrain, water, figures)
  // and the sprite layer (posed characters, battle cards) each hold the
  // depth of what they were drawn with, and a fragment is shadowed when
  // either has something closer to the sun. That is exactly the
  // single-map result -- step(z, min(a,b)) == step(z,a) * step(z,b) --
  ...
  float sunlight(vec3 p) {
    ...
    float z = p.z - sunBias;
    vec2 t = sunTexel;
    float lit = step(z, sunDepth(sunMap,  p.xy + t * vec2(-0.5, -0.5)))
              * step(z, sunDepth(sunMap2, p.xy + t * vec2(-0.5, -0.5)))
              + step(z, sunDepth(sunMap,  p.xy + t * vec2( 0.5, -0.5)))
              * step(z, sunDepth(sunMap2, p.xy + t * vec2( 0.5, -0.5)))
              + step(z, sunDepth(sunMap,  p.xy + t * vec2(-0.5,  0.5)))
              * step(z, sunDepth(sunMap2, p.xy + t * vec2(-0.5,  0.5)))
              + step(z, sunDepth(sunMap,  p.xy + t * vec2( 0.5,  0.5)))
...
function Voxel3D.beginScene(w, h, cx, cy, vw, vh, sky, slot)
  ...
  local map = ShadowMap.active(false)
  pcall(sh.send, sh, "sunVP", "row", map and ShadowMap.uvVP or IDENTITY)
  local tex = ShadowMap.texture(false)
  if tex then pcall(sh.send, sh, "sunMap", tex) end
  -- the CAST layer (sprites): sampled and ANDed with the world layer in
  -- sunlight() -- a fragment is lit only when neither layer has something
  -- closer to the sun. Sent unconditionally like sunMap: the sampler is
  -- declared either way, and unbound is a driver-dependent crash.
...
-- changing their projected screen position (unlike increasing SHADOW_EPS).
Voxel3D.SHADOW_PULL = 6
...
-- fractional while walking and camera blends can select different sprite
-- frames; neither should move a fixed low-end blob by a subpixel amount.
function Voxel3D.shadowPosition(px, py)
