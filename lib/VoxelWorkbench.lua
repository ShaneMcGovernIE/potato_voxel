-- Local browser-workbench bridge for Gold.  LuaSocket is intentionally not a
-- dependency of the game, so the browser server and game exchange tiny JSON
-- files in LOVE's save directory.  The server is localhost-only; this module
-- never opens a network listener or reads arbitrary filesystem paths.
...
local function reloadGold(game)
  ...
  -- HotReload's built-in map refresh is Gen 1's overworld controller.  Gold
  -- owns a World directly, so rebuild the live map after the new mod profile
  -- has been loaded and the mesh caches flushed.
...
-- The workbench needs to show the exact tiles the game is using, not numeric
-- ids.  In particular, Gold replaces part of the Johto atlas with each town's
-- roof art.  Asking World for its live atlas captures that composite and keeps
-- the browser's 16px cells faithful to New Bark, Violet, and so on.
local function exportAtlas(world, map)
  ...
  -- `Image` objects cannot be read back on every LÖVE target.  Recreate the
  -- same inexpensive atlas operation World:atlasFor performs: base tiles plus
  -- the nine map-group roof tiles.  The browser then sees the same artwork as
  -- the map, without depending on GPU readback support.
...
local function captureGraphic(game, command, target)
  ...
  -- A 24-cell span is already a 48x48 graphic tile template.  It protects
  -- status.json from an accidental whole-route capture while covering every
  -- ordinary overworld building.
...
local function writeStatus(game)
  ...
  if love.filesystem.write(STATUS_TMP, body) then
    -- LOVE's virtual filesystem does not expose atomic rename across every
    -- target, but a complete temporary write means a browser never observes
    -- a half-encoded status file in normal desktop use.
...
-- Automated driver runs deliberately call Game:update directly so every
-- scripted action owns exactly one simulation step.  render.compose is still
-- reached, so use it as a fallback for the bridge without double-polling a
-- normal interactive frame that already passed through core.update.
...
function Workbench.install()
  -- command.json is deliberately persistent so a browser and game can start
  -- in either order.  On a fresh loader, though, its last command has already
  -- happened; replaying an old `reload` during boot re-enters HotReload while
  -- registries are still being installed.  Remember that id as the baseline
  -- and process only commands the browser writes after this bridge is live.
  local raw = love.filesystem.read(COMMAND_PATH)
