...
-- FALLBACK ONLY (see castShadows below). Draw a fixed contact/blob decal.
-- Do not use the actor's animated card here: frame, mirror and lift all made
-- the old silhouette shimmer while walking or while the camera blended.
local function drawShadow(sprite, px, py, facing, phase, flip, gh, lift)
  local mesh, texture = SpriteBillboards.shadowBlob()
  if not mesh then return end
  -- Keep the decal depth-tested so walls still occlude it, but pull it
  -- camera-ward along each fragment ray. This is a depth-only bias: unlike
  -- lifting the blob in world Y, it cannot make the footprint swim as the
  -- perspective camera or curve changes.
  Voxel3D.draw(mesh, texture, Voxel3D.shadowBlobMatrix(px, py, gh),
               Voxel3D.SHADOW_PULL)
end
...
local function billboardMatrix(px, py, y, mirror)
  ...
  if b > 0 then
    Mat4.rotateYInPlace(Mat4.scratch1, FirstPerson.cardYaw(px + 8, py + 8) * b)
    Mat4.mulInPlace(billboardMat, billboardMat, Mat4.scratch1)
  end
  Mat4.rotateXInPlace(Mat4.scratch1, (leanAngle() - math.pi / 2) * (1 - b))
  Mat4.mulInPlace(billboardMat, billboardMat, Mat4.scratch1)
  if mirror then
    Mat4.scaleInPlace(Mat4.scratch1, -1, 1, 1)
    Mat4.mulInPlace(billboardMat, billboardMat, Mat4.scratch1)
  end
  Mat4.translateInPlace(Mat4.scratch1, -8, 0, 0)
  Mat4.mulInPlace(billboardMat, billboardMat, Mat4.scratch1)
  return billboardMat
end
...
local function figureMatrix(f, offX, offZ)
  ...
  Mat4.translateInPlace(figureMat, wx, f.y, wz)
  if b > 0 and f.w and f.w > 0 then
    local half = f.w / 2
    Mat4.translateInPlace(Mat4.scratch1, half, 0, 0)
    Mat4.mulInPlace(figureMat, figureMat, Mat4.scratch1)
    Mat4.rotateYInPlace(Mat4.scratch1, FirstPerson.cardYaw(wx + half, wz) * b)
    Mat4.mulInPlace(figureMat, figureMat, Mat4.scratch1)
    Mat4.translateInPlace(Mat4.scratch1, -half, 0, 0)
    Mat4.mulInPlace(figureMat, figureMat, Mat4.scratch1)
  end
...
local function figureCaster(f, offX, offZ)
  Mat4.translateInPlace(figureCasterMat, f.wx + (offX or 0), f.y, f.wz + (offZ or 0))
  Mat4.scaleInPlace(Mat4.scratch1, 1, 1, 0)
  return Mat4.mulInPlace(figureCasterMat, figureCasterMat, Mat4.scratch1)
end
...
local function posesOf(state, spriteColors, voxelPalette)
  ...
  for _, g in ipairs(state.ghosts or {}) do
    ...
    posed[#posed + 1] = {
      ...
      -- `pose()` may apply a visual walk/idle offset. Keep the contact
      -- decal on the entity's grounded collision position instead of making
      -- that render-only offset part of the shadow transform.
      anchorX = g.npc.px + g.ox, anchorY = g.npc.py + g.oy,
  ...
  for _, e in ipairs(state.entities or {}) do
    if not (state.flyAnim and e == state.player) then
      ...
      posed[#posed + 1] = {
        ...
        -- The posed sprite can be nudged for animation. Contact shadows are
        -- grounded decals, so anchor them to the collision body's position.
        anchorX = e.px, anchorY = e.py,
        facing = facing, phase = phase, flip = flip,
...
--
-- TWO LAYERS, because the two kinds of caster move on different clocks:
-- the WORLD layer (terrain, neighbours, water, flowers, figures) changes
-- only when the camera, the sun or the world does, while the SPRITE layer
-- (posed characters, battle cards) animates every frame a character idles
-- or walks. The world layer is re-rasterised only when ITS signature
-- moves; the sprite layer only when a sprite's does. A standing player
-- (or a dialog, or a menu -- the sprite layer still animates behind it)
-- no longer re-draws the whole world from the sun on every frame, only
-- the handful of cards. The main pass samples both layers and ANDs the
-- depth tests, which is exactly the single-map result: a fragment is lit
...
local function castShadows(state, terrain, ring, nbMesh, posed, cx, cy, vw, vh,
  ...
  -- The profile owns the quality decision. HIGH keeps the two-layer actor
  -- map on every device; MEDIUM and lower use the cheaper contact/blob
  -- fallback while the world layer follows the active device policy.
  local spriteLayer = BrickProfile.actorShadowMapEnabled(Voxel.level)
  ShadowMap.setSpriteLayerActive(spriteLayer)
  spriteLayer = spriteLayer and ShadowMap.spriteLayerEnabled()
  local spriteStale = spriteLayer and ShadowMap.stale(spriteSig, true) or false
  ...
  if worldStale then
    ...
    local ok, err = pcall(function()
      ...
      -- a staged fight's discs, if one is standing: geometry, not
      -- cut-outs -- un-snugged, and drawn OUTSIDE the sprite flag exactly
      -- as the battle pass draws them, so water receives them like every
      -- other world caster.
      pcall(function()
        local stageArena, stageY = V.require("OverworldBattle").stage()
        if stageArena and stageArena.discs then
          V.require("DiscArena").cast(ShadowMap, stageArena, stageY or 0)
        end
      end)
  ...
  if battleTex then
    ...
    if okB and cards then
      ...
      pcall(function()
        local stageArena, stageY = V.require("OverworldBattle").stage()
        if stageArena and stageArena.discs then
          V.require("DiscArena").draw(stageArena, stageY or 0)
        end
      end)
