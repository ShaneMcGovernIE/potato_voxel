...
-- A field (not a local) so BrickProfile can drop it to 0 on the TrimUI
-- Brick -- that collapses the whole ring to the existing black-void
-- treatment and cuts the dominant geometry term on a ~1GB device.
-- Must stay a multiple of 4 so no 2x2-cell canopy group is left
-- half-resolved at the edge.
Structures.ROUND_RING = 4

-- BILLBOARD HULLS (Brick): the round carving collapses to a single
-- south-facing flat card cut to the mask silhouette. The Brick's orbit
-- camera is fixed south (no yaw), so one card per hull is geometrically
-- correct at every pitch; the tileset art carries no alpha, so the card
-- silhouette comes from the mask itself -- one quad per contiguous mask
-- run per row, at the hull's front plane. This deletes the back faces,
-- side walls, flat tops and foot rows in one stroke (the h=1 8px-tall
-- perimeter side walls alone were 64% of Viridian's quads). A field (not
-- a local) so BrickProfile can flip it on the TrimUI Brick.
Structures.HULL_BILLBOARDS = false

-- SPRITE CROSSHAIR for billboard hulls (Brick only; the full desktop
-- carve ignores this). True = repeat the card at +45 and -45 degrees
-- about the hull's vertical axis, forming an X cross in plan -- the
-- crossed-billboard dome. False = the single flat card.
...
function Structures.forMap(map)
  ...
  local function tileLookup(tx, ty)
    ...
    if hullRingOnly and (tx < -Structures.ROUND_RING or ty < -Structures.ROUND_RING
                         or tx >= tw2 + Structures.ROUND_RING
                         or ty >= th2 + Structures.ROUND_RING) then
      return nil
    end
...
local function roundTemplate(S, map, data, cx, cy, groundTiles, N, capRows,
  ...
  -- Shared by the billboard branch (on the full mask) and the carved
  -- hull below (on the post-strip mask).
  local function scoreGround(baseArt)
    local bg = nil
  ...
  -- BILLBOARD HULLS (the TrimUI Brick): the whole round carve -- front
  -- and back faces, perimeter side walls, flat tops, foot rows -- is the
  -- dominant geometry term on the Brick, and the Brick's orbit camera is
  -- FIXED SOUTH: pitch-only (35 deg), no yaw, so every hull is always
  -- seen from the same angle. That makes the carve replaceable by ONE
  -- flat, south-facing card at the hull's front plane, cut to the mask
  -- silhouette. The tileset carries no alpha (measured: every overworld
  -- pixel is a=255), so the card's shape cannot come from alpha discard
  -- -- it comes from the MASK itself: each mask row emits one quad per
  -- contiguous run (a run never crosses an 8px atlas-tile seam, so its u
  -- range interpolates inside one tile), wearing its own row's texels.
  -- Cap/base stripping and the disc carve are skipped entirely, so the
  -- card shows the FULL drawing -- cut face, base rim and all -- at its
  -- drawn proportions. A tree costs ~a dozen quads instead of ~3000.
  --
  -- SPRITE CROSSHAIR (Brick, BILLBOARD_CROSS): the flat card reads
  -- paper-thin from the 35-degree camera, and the Brick's camera is
  -- yaw-locked due south -- so the classic 90-degree billboard cross
  -- (a card facing east) would be edge-on and INVISIBLE. What reads is
  -- the X cross: the same card repeated at +45 and -45 degrees about
  -- the hull's vertical center axis. Each diagonal projects at cos(45)
  -- = 70.7% width and its far-side top edge rises a step above the
  -- front card's, so the canopy silhouette gains slanted shoulders
  -- instead of a flat top -- the crossed-billboard dome of the N64-era
  -- trees. Cost: 3x the flat card (~36 quads a tree), still ~80x
  -- cheaper than the carve. The depth buffer resolves the overlaps
  -- (the front card owns the center; the diagonals own the shoulders).
  if Structures.HULL_BILLBOARDS then
    local quads = {}
    local zF = N2
    -- cross arms at +45 / -45 about the hull center (x=0, z=zF):
    -- x' = x*cos, z' = zF + x*sin. cos(45) = sin(45) = 0.7071.
    local cross = Structures.BILLBOARD_CROSS
    local C, S = 0.7071067811865476, 0.7071067811865476
    for iy = 0, NY - 1 do
      local lo, hi = nil, nil
      for ix = 0, NX - 1 do
      ...
      if lo then
        ...
        while ix <= hi do
          ...
          if mask[i] then
            local ix2 = ix
            while ix2 + 1 <= hi and mask[iy * NX + ix2 + 1]
                  and math.floor((ix2 + 1) / 8) == math.floor(ix / 8) do
              ix2 = ix2 + 1
            end
            local x0, x1 = ix - N2, ix2 - N2 + 1
            local ax0 = texel(ix, iy)
            local ax1 = texel(ix2, iy)
            local u0, u1 = (ax0 + 0.05) / atlasW, (ax1 + 0.95) / atlasW
            -- layer 0 is the existing front card, byte-identical
            quads[#quads + 1] = {
            ...
            if cross then
              -- +45 arm: right edge swings south (toward the camera),
              -- left edge north. Its normal points southeast -- toward
              -- the sun (Voxel3D: "the sun hangs in the SOUTHEAST") --
              -- so it wears a near-front shade; the -45 arm's normal
              -- points southwest, away from the sun, so it wears the
              -- side shade. The lit/unlit pair is what makes the cross
              -- read as a rounded dome rather than two flat cards.
              quads[#quads + 1] = {
                { x0 * C, yB, zF + x0 * S }, { x1 * C, yB, zF + x1 * S },
                { x1 * C, yT, zF + x1 * S }, { x0 * C, yT, zF + x0 * S },
                uv = { { u0, v1 }, { u1, v1 }, { u1, v0 }, { u0, v0 } },
                shade = 0.95,
              }
              -- -45 arm: mirrored
  ...
  -- the ground the ball stands on (shared scorer above): the drawing's
  -- own background names it. A stripped base row is the OBJECT's own rim,
  -- not background -- scoring its whites against the floor tiles matches
  -- paper-white ground under a can whose art stands on the gym's grey.
  local bg = scoreGround(baseArt)
